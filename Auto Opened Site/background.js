...
chrome.tabs.onUpdated.addListener((tabId, changeInfo, tab) => {
  if (changeInfo.status !== 'complete') return; // Only proceed if the tab update is complete
  if (!tab.url) return; // Ensure the tab has a URL
  ...
  chrome.storage.sync.get(['triggerSite', 'destinationSite', 'titleKeywords'], (data) => {
    ...
    else if (titleKeywords && tab.url.includes("youtube.com/watch") && destinationSite) {
